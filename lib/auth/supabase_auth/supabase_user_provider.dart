...
class MyNoteSupabaseUser extends BaseAuthUser {
  MyNoteSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> myNoteSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MyNoteSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
